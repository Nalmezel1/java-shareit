...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDtoResponse create(BookingDtoRequest bookingDtoRequest, long bookerId) {
        ...
        if (item.getOwner().getId() == bookerId) {
            throw new NotFoundException("ельзя брать свою вещь");
        }
        if (!item.getAvailable()) {
            throw new ValidationException("не доступно");
        }
    ...
    public BookingDtoResponse updateStatus(long bookingId, long userId, boolean isApprove) {
        ...
        if (booking.getStatus().equals(WAITING)) {
            ...
            throw new ValidationException("без изменений");
        }
    ...
    private Booking getBookingOrThrow(long bookingId) {
        return bookingRepository.findById(bookingId)
                .orElseThrow(() -> new NotFoundException("error" + bookingId));
    }

    private User getUserOrThrow(long userId) {
        return userRepository.findById(userId)
                .orElseThrow(() -> new NotFoundException("error" + userId));
    }

    private Item getItemOrTrow(long itemId) {
        return itemRepository.findById(itemId)
                .orElseThrow(() -> new NotFoundException("error" + itemId));
    }
